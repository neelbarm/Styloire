...
export function StyloireMarketingHeader() {
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/30 bg-black/10 px-8 py-5 backdrop-blur-[2px] md:px-14">
        ...
        >
          {centerLinks.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full border border-white/45 bg-white/10 px-6 py-2 font-sans text-[0.65rem] font-semibold uppercase tracking-[0.13em] text-white/90 transition-[color,background-color,border-color] duration-styloire ease-styloire hover:border-white/70 hover:bg-white/20"
            >
              {item.label}
            </Link>
          ))}
